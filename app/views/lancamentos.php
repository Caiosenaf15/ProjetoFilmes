...
$lancamentos = $tmdb->futurosLancamentos($page, $adult, $order);
?>
...
<div class="movies-grid">
    <?php foreach ($lancamentos['results'] as $lan): 
    ...
    ?>
        <div class="movie-card"
            onclick="modalOpen(
                '<?= $backdrop ?>',
                '<?= htmlspecialchars(addslashes($lan['title'])) ?>',
                '<?= htmlspecialchars(addslashes($lan['overview'])) ?>',
                '<?= htmlspecialchars($tmdb->ondeAssistir($lan['id'])) ?>',
                '<?= htmlspecialchars($lan['release_date']) ?>'
            )">
...
<?php
$totalPages = $lancamentos['total_pages'];
$maxLinks = 5;
$start = max(1, $page - 2);
...
function modalOpen(backdrop, title, overview, stream, data) {
